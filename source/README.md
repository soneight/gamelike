...
- `Action`: _main thread window creation and event handling (maybe also scripting and `GUI`)_
- `Direct`: _back thread planner, data translator between threads below (but can also run all or some of them on same thread):_
...
So system always be running on at least two threads (`Action` plus `Direct`) and up to eight threads which depends on `Direct` configuration andor inner `Direct` logic for splitting work
