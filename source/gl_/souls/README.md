...
as based mainly on `Dark Souls 2` there several major changes:

- maximum level is `64` on first play through and `256` on `NG+7`
- no adaptability as standalone attribute, it now scales from levels and depends on `Dual` stat duo (see `Attributes`)
- overall game should feel around 20 percent faster

### Attributes
> most of attributes names should be changed
