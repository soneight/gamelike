...
- `Aggro` (Yellow): like `gruul` form `MTG` or `Shaman` form `HS`
- `Mezzo` (Cyan): like `simic` from `MTG` or `Paladin` from `HS`
- `Tempo` (Magenta): like `izzet` from `MTG` or `Mage` from `HS`
- `Combo` (White): like `esper` but assembles `red+green+blue` mechanics from `MTG` or `Rogue` from `HS`
